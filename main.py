...
class TextureFunction(nn.Module):
    ...
    def forward(self, X, Y):
        R = torch.sin(self.alpha * X) + torch.cos(self.phi_R * Y) 
        G = torch.sin(self.beta * X) + torch.cos(self.phi_G * Y) 
        B = torch.sin(self.gamma * X) + torch.cos(self.phi_B * Y) 
...
if __name__ == "__main__":
    ...
    h, w = 30, 30  # Example size of the patch
